...
import { Opt } from '@/support/option';
...
function t(dark: Opt<string>, light: Opt<string>): Accessor<string>;
function t(dark: string, light: string): Accessor<string>;
function t(dark: Opt<string> | string, light: Opt<string> | string) {
  if (typeof dark === 'string' && typeof light === 'string') return scheme.mode()(s => (s === 'dark' ? dark : light));
  if (dark instanceof Opt && light instanceof Opt) return createComputed(track => (track(scheme.mode()) === 'dark' ? dark : light));
}
