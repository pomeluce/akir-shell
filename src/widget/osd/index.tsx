...
function anchor(anchors: Array<'top' | 'bottom' | 'left' | 'right'>) {
  return anchors.reduce((prev, a) => prev | Astal.WindowAnchor[a.toUpperCase() as 'TOP' | 'BOTTOM' | 'LEFT' | 'RIGHT'], 0);
}

function OnScreenProgress({ vertical, visible, setVisible }: { vertical: boolean; visible: Accessor<boolean>; setVisible: Setter<boolean> }) {
  ...
  return (
    <revealer
      $={() => {
        ...
        brightness.connect('notify::kbd', () => show(brightness.kbd, icons.keyboard));
        speaker.connect('notify::volume', () => show(speaker.volume, speaker.volumeIcon));
      }}
      revealChild={visible}
